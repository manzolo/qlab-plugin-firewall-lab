...
FIREWALL_PORT="$(_get_port firewall-lab-firewall)"
ATTACKER_PORT="$(_get_port firewall-lab-attacker)"
if [[ -z "$FIREWALL_PORT" || -z "$ATTACKER_PORT" ]]; then echo "ERROR: Cannot find VM ports. Are firewall-lab VMs running?"; exit 1; fi

_ssh_base_opts=(-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR)
ssh_firewall() { ssh "${_ssh_base_opts[@]}" -i "$SSH_KEY" -p "$FIREWALL_PORT" labuser@localhost "$@"; }
ssh_attacker() { ssh "${_ssh_base_opts[@]}" -i "$SSH_KEY" -p "$ATTACKER_PORT" labuser@localhost "$@"; }

# Get service ports from .ports file
_get_service_port() {
    local vm="$1" guest_port="$2"
    local f="$STATE_DIR/${vm}.ports"
    if [[ -f "$f" ]]; then
        grep ":${guest_port}$" "$f" | head -1 | cut -d: -f2
    else
        echo ""
    fi
}

HTTP_PORT="$(_get_service_port firewall-lab-firewall 80)"
DASHBOARD_PORT="$(_get_service_port firewall-lab-firewall 9090)"
MYSQL_PORT="$(_get_service_port firewall-lab-firewall 3306)"
