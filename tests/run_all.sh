...
echo ""; echo "${BOLD}=========================================${RESET}"; echo "${BOLD}  firewall-lab — Automated Test Suite${RESET}"; echo "${BOLD}=========================================${RESET}"; echo ""
log_info "Firewall VM port: $FIREWALL_PORT"; log_info "Attacker VM port: $ATTACKER_PORT"
if [[ -n "$HTTP_PORT" ]]; then log_info "HTTP port: $HTTP_PORT"; fi
if [[ -n "$DASHBOARD_PORT" ]]; then log_info "Dashboard port: $DASHBOARD_PORT"; fi
if [[ -n "$MYSQL_PORT" ]]; then log_info "MySQL port: $MYSQL_PORT"; fi
assert "Firewall VM reachable" ssh_firewall "echo ok"; assert "Attacker VM reachable" ssh_attacker "echo ok"
